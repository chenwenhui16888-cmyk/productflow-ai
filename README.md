...
11. **生成任务状态与 AI 可观测性**：持久化记录运行中、成功和失败状态，并展示模型、耗时、尝试次数、自动修复及降级信息。
12. **PRD 独立导出**：仅导出最终 PRD，不混入需求澄清、原型和内部生成日志。
...
执行以下命令可创建一套完整且可重复的“智能会议行动项助手”演示数据：

```powershell
npm.cmd run demo:seed
```

案例包含六个澄清答案、四条需求、三条用户故事、三张低保真页面、完整 PRD、研发交付清单、评审报告和一份项目知识资料。脚本会先删除同名旧案例，避免重复数据。面试时建议展示该案例，再现场重新生成一个关键步骤。
...
1. 模型首次生成 JSON。
2. 解析并执行结构校验。
3. 失败时携带原始输出和失败类型自动修复一次。
4. 修复仍失败时使用规则模板降级。

`GenerationLog.outputSnapshot.metrics` 会记录耗时、尝试次数、首次解析结果、首次校验结果、修复结果、错误类型和 RAG 检索片段数量。

工作台右侧的 AI 运行面板会轮询持久化任务状态。页面刷新后仍能看到运行中或失败任务，最近生成记录可用于解释模型调用、自动修复和规则降级过程。

知识库管理支持查看处理状态、文本块数量和更新时间，并可重新索引或删除资料。删除资料会级联清理对应文本块和向量。
